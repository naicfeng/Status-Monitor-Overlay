...
private:
    ...
    {
        if (delta > 20000)
            return diffSymbols[0]; // △
        if (delta > -20000)
            return diffSymbols[1]; // @
        if (delta < -50000)
            return diffSymbols[3]; // ≠
        return diffSymbols[2];     // ▽
    }

    void calculateLayoutMetrics(tsl::gfx::Renderer *renderer)
    {
        if (layout.calculated)
            return;
            ...
            for (size_t i = 0; i < all_items_ordered.size(); i++) {
                ...
                if (settings.useDynamicColors) {
                    ...
                    if (item.type == 3) { // SOC temperature
                        // Parse SOC temperature: support either "XX°C (XX%)" or single-character "XX℃ (XX%)"
                        std::string dataStr(item.data_ptr);
                        const std::string degSymbol = "\u2103"; // U+2103

                        // Try to find single-character ℃ first, otherwise fall back to '°' + 'C'
                        const size_t degSymPos = dataStr.find(degSymbol);
                        size_t tempEnd = std::string::npos;
                        bool parsed = false;

                        if (degSymPos != std::string::npos) {
                            tempEnd = degSymPos + degSymbol.size(); // bytes length of the symbol
                            parsed = true;
                        } else {
                        ...
                        // Parse TMP temperatures: support either "XX°C XX°C XX°C (XX%)" or "XX℃ XX℃ XX℃ (XX%)"
                        std::string dataStr(item.data_ptr);
                        const std::string degSymbol = "\u2103"; // U+2103
                        ...
                        for (int tempCount = 0; tempCount < 3 && parseSuccess && pos < dataStr.length(); tempCount++) {
                            ...
                            // Try to find single-character ℃ first at or after pos
                            size_t degreesPos = dataStr.find(degSymbol, pos);
                            bool useSingleSymbol = true;

                            if (degreesPos == std::string::npos) {
                                // Fall back to legacy '°' + 'C' format
                                degreesPos = dataStr.find("°", pos);
